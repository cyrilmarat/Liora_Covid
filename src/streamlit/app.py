...
Le vrai problème ici : Streamlit Cloud utilise Python 3.14.6, une version beaucoup trop récente pour TensorFlow — le résolveur ne trouve aucune wheel TensorFlow compatible avec cette version de Python, donc aucune version de TensorFlow n'est installable. TensorFlow n'a pas encore de builds pour Python 3.14 (dans les faits, il supporte généralement jusqu'à 3.11 ou 3.12 selon la version).

Il faut donc forcer une version de Python plus ancienne pour votre app. Vérifions la méthode actuelle recommandée par Streamlit Cloud.
Web recherché

C'est un problème connu et documenté actuellement sur les forums Streamlit — pas propre à votre app.
Diagnosed runtime bug et préconisa redéploiement avec version Python explicite

Vous avez touché un bug de plateforme actuellement documenté et non résolu (mai 2026) :
Diagnostic

Streamlit Community Cloud ne respecte PAS les fichiers runtime.txt ou .python-version pour choisir la version de Python — la seule méthode officiellement supportée est le menu « Advanced settings » lors du déploiement. Cependant, plusieurs utilisateurs signalent que même en sélectionnant Python 3.11 dans ce menu, la plateforme utilise quand même Python 3.14.x — ce qui semble être un bug côté Streamlit ou un décalage dans le déploiement des nouvelles versions de Python.

TensorFlow n'a tout simplement pas encore de wheels compatibles avec Python 3.14 — d'où votre erreur.
Ce qu'il faut essayer

1. Supprimer et redéployer l'app en forçant Python 3.11 ou 3.12 — vous ne pouvez pas changer la version de Python d'une app déjà déployée ; il faut la supprimer puis la redéployer en choisissant la version voulue dans "Advanced settings" au moment du déploiement.
Streamlit

Étapes :

    Dans le dashboard Streamlit Cloud, allez sur votre app → Delete app
    Cliquez sur Deploy an app → renseignez le repo/branch/src/streamlit/app.py
    Dans Advanced settings, sélectionnez explicitement Python 3.11 (ou 3.12) avant de cliquer sur Deploy

2. Si le bug persiste malgré ça (cas rapporté par plusieurs personnes récemment) — c'est un problème de plateforme, pas de votre code. Deux options :

    Réessayer dans quelques jours (les rollouts buggés se corrigent généralement vite chez Streamlit)
    Poster vous-même sur le forum officiel (discuss.streamlit.io) pour signaler que ça vous touche aussi — ça aide à faire remonter la priorité du bug

3. En parallèle, pendant que vous y êtes, pinnez une version stable de TensorFlow compatible 3.11/3.12 dans votre requirements.txt pour éviter de retomber sur des soucis de résolution :
txt
...
Voulez-vous que je vous aide à identifier une alternative de déploiement (Hugging Face Spaces, Render...) si le bug Streamlit Cloud persiste trop longtemps ?

j'ai ces traces maintenant :
...
File "/mount/src/liora_covid/src/streamlit/app.py", line 146, in <module>
    st.sidebar.image("intro.png", width=400)

File "/home/adminuser/venv/lib/python3.12/site-packages/streamlit/runtime/metrics_util.py", line 568, in wrapped_func     result = non_optional_func(*args, **kwargs)              ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
File "/home/adminuser/venv/lib/python3.12/site-packages/streamlit/elements/image.py", line 217, in image     marshall_images(
File "/home/adminuser/venv/lib/python3.12/site-packages/streamlit/elements/lib/image_utils.py", line 447, in marshall_images     proto_img.url = image_to_url(                     ^^^^^^^^^^^^^
File "/home/adminuser/venv/lib/python3.12/site-packages/streamlit/elements/lib/image_utils.py", line 306, in image_to_url     url = runtime.get_instance().media_file_mgr.add(image, mimetype, image_id)           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
File "/home/adminuser/venv/lib/python3.12/site-packages/streamlit/runtime/media_file_manager.py", line 277, in add     file_id = self._storage.load_and_get_id(               ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
File "/home/adminuser/venv/lib/python3.12/site-packages/streamlit/runtime/memory_media_file_storage.py", line 155, in load_and_get_id     self._read_file(path_or_data)
File "/home/adminuser/venv/lib/python3.12/site-packages/streamlit/runtime/memory_media_file_storage.py", line 208, in _read_file     raise MediaFileStorageError(f"Error opening '{filename}'") from ex
Diagnosed path resolution issue and architected centralized directory solution

Bonne nouvelle : votre problème TensorFlow/Python est résolu (Python 3.12 tourne maintenant, l'installation passe). Le nouveau souci est différent et classique lors du déploiement.
Diagnostic

st.sidebar.image("intro.png", width=400) échoue parce que le chemin "intro.png" est relatif au répertoire de travail (cwd), pas au fichier app.py. En local, quand vous lancez streamlit run app.py depuis src/streamlit/, le cwd est ce dossier, donc "intro.png" s'y trouve. Sur Streamlit Cloud, le répertoire de travail est toujours la racine du dépôt — donc "intro.png" est cherché à la racine de Liora_Covid/, où il n'existe pas.
Streamlit

Ce problème touche potentiellement TOUS les chemins relatifs de votre script : "distribution.png", "hist_intensite_1.png", "preprocessing_1_brute.png", mais aussi les chemins comme model_CNN_path = "../../models/cnn256/cnn_256.keras" qui reposent sur le même principe.
Solution robuste : construire les chemins depuis l'emplacement du script

Ajoutez en haut de app.py :
python

from pathlib import Path

BASE_DIR = Path(__file__).resolve().parent          # = .../src/streamlit
PROJECT_ROOT = BASE_DIR.parent.parent                # = racine du repo

Puis remplacez les chemins relatifs par des chemins construits à partir de ces bases. Par exemple :
python
...
Et pour toutes les images (st.sidebar.image, st.image) :
python

# Avant
st.sidebar.image("intro.png", width=400)
...
st.image("distribution.png", caption="...", width=650)

# Après
st.sidebar.image(str(BASE_DIR / "intro.png"), width=400)
...
st.image(str(BASE_DIR / "distribution.png"), caption="...", width=650)

Les chemins déjà écrits comme "../../reports/figures/exemples_images_masques.png" doivent devenir str(PROJECT_ROOT / "reports" / "figures" / "exemples_images_masques.png").

Cette approche fonctionne à l'identique en local et sur Streamlit Cloud, quel que soit le répertoire depuis lequel vous lancez la commande.

Voulez-vous que je vous génère la version corrigée complète du fichier app.py, avec tous les chemins remplacés d'un coup ?

oui
Web recherché, a exécuté une commande
GitHub - usnistgov/cv-py: Collection of tools and techniques to kick-start analysis of the COVID-19 Research Challenge Dataset · GitHub
github.com
covid19nsw/app.py at master · pwilson802/covid19nsw
github.com
COVID-19/app.py at main · Pinak18/COVID-19
github.com
Covid19_VA_Project/app.py at master · rajshah1/Covid19_VA_Project
github.com
COVID-19-Tracker/app.py at main · mmeadx/COVID-19-Tracker
github.com
GitHub - open-coronavirus/open-coronavirus: Open Coronavirus · GitHub
github.com
covid-ai-demo-deployment/app.py at master · zhongli1990/covid-ai-demo-deployment
github.com
covid-19/app.py at master · nite/covid-19
github.com
COVID Tracker Ireland
en.wikipedia.org
covid-dashboard/app.py at master · covid19-dash/covid-dashboard
github.com
Terminé

Récupération du fichier exact réussie (formatage préservé). Je l'inspecte puis j'applique les corrections de chemins.
Viewing file

Parfait, le formatage est intact. Je copie le fichier et applique toutes les corrections de chemins.
Orchestrated systematic code refactoring with path definitions

Maintenant j'applique toutes les corrections.
18 fichiers modifiés, a consulté 2 fichiers

Vérification finale : aucun chemin relatif ne doit rester.
Vérifier qu'aucun chemin relatif ne subsiste et que le fichier compile

Tous les chemins sont désormais construits dynamiquement, et le fichier compile sans erreur. Je le mets à disposition.
A exécuté une commande, lu un fichier

Résumé des changements :

    Ajout de BASE_DIR = Path(__file__).resolve().parent (dossier src/streamlit/) et PROJECT_ROOT = BASE_DIR.parent.parent (racine du dépôt) juste après les imports.
    Les 7 chemins de modèles/données (model_CNN_path, val_dir, test_dir, model_SVM_path, scaler_SVM_path, csv_test, csv_validation) sont désormais construits depuis PROJECT_ROOT.
    Les 23 appels st.image/st.sidebar.image/image_to_data_uri/pd.read_csv utilisant un chemin relatif sont corrigés : ceux pointant vers des fichiers situés à côté d'app.py utilisent BASE_DIR, ceux avec ../../reports/... ou ../../models/... utilisent PROJECT_ROOT.

À faire de votre côté :

    Remplacez src/streamlit/app.py par ce fichier dans votre dépôt (git add, git commit, git push).
    Vérifiez que les images référencées par nom simple (intro.png, distribution.png, hist_intensite_1.png, etc.) sont bien présentes dans src/streamlit/ sur GitHub — si elles étaient seulement en local et pas commitées, elles manqueront aussi sur Cloud.
